  ...
  {#if edit}
    <div class="package_tmpl_top border rounded p-3 my-2">
      <form action="" on:submit|preventDefault={submitEdit}>
        ...
        <input id="sort_key_input" type="number" min={Number.MIN_SAFE_INTEGER} max={Number.MAX_SAFE_INTEGER} step="1" name="sort_key" class="package_tmpl_name form-control" required on:input={validatePackageSortKey}>
